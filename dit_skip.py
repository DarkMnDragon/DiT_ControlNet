...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.out_blocks = nn.ModuleList([
            DiTBlock(hidden_size=hidden_size, num_heads=num_heads, mlp_ratio=mlp_ratio, skip=use_long_skip)
            for _ in range(depth // 2)])

        self.final_layer = FinalLayer(hidden_size, patch_size, self.out_channels)
        self.final_conv = nn.Conv2d(out_channels, out_channels, 3, padding=1) if final_conv else nn.Identity()
